using Npgsql;
using Modul3PAA.Helper;

namespace Modul3PAA.Models
{
    public class PersonContext
    {
        ...
        public List<Person> ListPerson()
        {
            ...
            string query = string.Format(@"SELECT id_person, nama, alamat, email FROM siswa order by id_person;");
        ...
        {
            string query = @"INSERT INTO siswa (nama, alamat, email) VALUES (@nama, @alamat, @email)";
            sqlDBHelper db = new sqlDBHelper(this.__constr);
        ...
        {
            string query = @"delete from siswa where id_person = @id_person";
            sqlDBHelper db = new sqlDBHelper(this.__constr);
            try
            {
                NpgsqlCommand cmd = db.getNpgsqlCommand(query);
